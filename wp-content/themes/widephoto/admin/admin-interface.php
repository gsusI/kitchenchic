...
$functions_path = wdp_FILEPATH . '/admin/';

function zillaframework_add_admin() {

    global $query_string;
    
    $themename =  get_option('wdp_themename');      
    $shortname =  get_option('wdp_shortname'); 
   
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'zillaframework' ) {
		if (isset($_REQUEST['wdp_save']) && 'reset' == $_REQUEST['wdp_save']) {
			$options =  get_option('wdp_template'); 
			wdp_reset_options($options,'zillaframework');
			header("Location: admin.php?page=zillaframework&reset=true");
			die;
		}
    }
				
	$wdp_page = add_theme_page( $themename, 'Theme Options', 'edit_theme_options', 'zillaframework','zillaframework_options_page');
	
	// Add framework functionaily to the head individually
	add_action("admin_print_scripts-$wdp_page", 'wdp_load_only');
	add_action("admin_print_styles-$wdp_page",'wdp_style_only');
} 
...
/*-----------------------------------------------------------------------------------*/
/* Options Reset Function - wdp_reset_options */
/*-----------------------------------------------------------------------------------*/

function wdp_reset_options($options,$page = ''){
	...
	//When Theme Options page is reset - Add the wdp_options option
	if($page == 'zillaframework'){
		$query_inner .= " OR option_name = 'wdp_options'";
	}
...
?>
	...
	<div class="wrap" id="wdp_container">
	...
	?>
	<div id="tz-admin-menu">
	<a href="<?php echo esc_url( __( 'http://buzzrain.com', 'widephoto' ) ); ?>" title="<?php esc_attr_e( 'Wordpress for Photographers', 'widephoto' ); ?>"><img src="<?php echo get_template_directory_uri(); ?>/images/buzzrain.png" width="150px" height="42px" /></a>
		<ul>
		<li><a href="<?php echo esc_url( __( 'http://buzzrain.com/theme/widephoto', 'widephoto' ) ); ?>" title="<?php esc_attr_e( 'Theme Help & Documentation', 'widephoto' ); ?>"><?php esc_attr_e('Theme Homepage & Help', 'widephoto' ); ?></a>
		</ul>
	</div>	
	<div id="main">
		<div id="tz-nav">
	...
	<input type="hidden" name="wdp_save" value="reset" />
	</span>
	</form>
</div>
...
/*-----------------------------------------------------------------------------------*/
/* Load required styles for Options Page - wdp_style_only */
/*-----------------------------------------------------------------------------------*/

function wdp_style_only() {
	wp_enqueue_style('admin-style', wdp_DIRECTORY.'/admin/admin-style.css');
	wp_enqueue_style('color-picker', wdp_DIRECTORY.'/admin/css/colorpicker.css');
}	


/*-----------------------------------------------------------------------------------*/
/* Load required javascripts for Options Page - wdp_load_only */
/*-----------------------------------------------------------------------------------*/

function wdp_load_only() {

	add_action('admin_head', 'wdp_admin_head');
	
	wp_enqueue_script('jquery-ui-core');
	wp_register_script('jquery-input-mask', wdp_DIRECTORY.'/admin/js/jquery.maskedinput-1.2.2.js', array( 'jquery' ));
	wp_enqueue_script('jquery-input-mask');
	wp_enqueue_script('color-picker', wdp_DIRECTORY.'/admin/js/colorpicker.js', array('jquery'));
	wp_enqueue_script('ajaxupload', wdp_DIRECTORY.'/admin/js/ajaxupload.js', array('jquery'));

}

function wdp_admin_head() {
?>
<style type="text/css">
	#tz-admin-menu {
		position:relative;
		background-color:#87CC94;
...
<script type="text/javascript" language="javascript">
		...
		if (typeof AjaxUpload != 'function') { 
		 return ++counter < 6 && window.setTimeout(init, counter * 500);
			...
			//Color Picker
			<?php $options = get_option('wdp_template');
...
<script type="text/javascript">
			...
			jQuery('#expand_options').click(function(){
				if(flip == 0){
					flip = 1;
					jQuery('#wdp_container #tz-nav').hide();
					jQuery('#wdp_container #content').width(755);
					jQuery('#wdp_container .group').add('#wdp_container .group h2').show();
	
					jQuery(this).text('[-]');
					
				} else {
					flip = 0;
					jQuery('#wdp_container #tz-nav').show();
					jQuery('#wdp_container #content').width(595);
					jQuery('#wdp_container .group').add('#wdp_container .group h2').hide();
					jQuery('#wdp_container .group:first').show();
					jQuery('#wdp_container #tz-nav li').removeClass('current');
					jQuery('#wdp_container #tz-nav li:first').addClass('current');
...
/*-----------------------------------------------------------------------------------*/
/* Ajax Save Action - wdp_ajax_callback */
/*-----------------------------------------------------------------------------------*/

add_action('wp_ajax_wdp_ajax_post_action', 'wdp_ajax_callback');

function wdp_ajax_callback() {
...
function zillaframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case "multicheck":
			...
			$wdp_key = $value['id'] . '_' . $key;
			$saved_std = get_option($wdp_key);
			...
			elseif( $std == $key) {
			   $checked = 'checked="checked"';
				...
				$checked = '';                                                                                    }
			$output .= '<input type="checkbox" class="checkbox tz-input" name="'. $wdp_key .'" id="'. $wdp_key .'" value="true" '. $checked .' /><label for="'. $wdp_key .'">'. $option .'</label><br />';
